package com.cla.muliple.status.layout
...
class MultipleStatusLayout(context: Context, attr: AttributeSet? = null) :
    ...
    private val defaultViewClickListener = OnClickListener {
    ...
    private val contentShowView = lazy {
        getView(contentView, contentViewLayoutId, contentViewParams, null)?.apply {
            initView?.invoke()
        }
    }
    private val loadingShowView = lazy {
        getView(loadingView, loadViewLayoutId, loadingViewParams, null)?.apply {
            initLoadView?.invoke(this)
        }
    }
    private val emptyShowView = lazy {
        getView(emptyView, emptyViewLayoutId, emptyViewParams, emptyViewClickListener)?.apply {
            initEmptyView?.invoke(this)
        }
    }
    private val errorShowView = lazy {
        getView(errorView, errorViewLayoutId, errorViewParams, errorViewClickListener)?.apply {
            initErrorView?.invoke(this)
        }
    }
...
//        setBackgroundResource(R.color.c11)
    ...
    fun useDefaultErrorView(context: Context, force: Boolean = false) {
        ...
        errorView = with(TextView(context)) {
            text = "数据加载失败，点我重试"
    ...
    /**
     * 显示数据加载错误提示页面
     * @param force 是否强制显示没有网络页面，如果为true，那么不管当前是否已经显示content，都会切换为无网络提示页面
     * 如果为false，那么当前页面为content时，不会切换
     */
    fun showNoNetwork(force: Boolean = false) {
